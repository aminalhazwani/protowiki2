...
- **Hand-authored article pages** (no `LiveArticle` / `ArticleSnapshot`): compose **`ChromeWrapper` → `ArticleWrapper` → `ArticleRenderer`** and put markup in the renderer slot; use **`section.hand-authored-lead`** when the lead includes an infobox so mobile matches enwiki order. See [`protowiki-components` → `article.md`](.agents/skills/protowiki-components/references/article.md) and **`src/prototypes/hand-written-article/`**.
