...
import globePiecesSvgRaw from './wikipedia-globe-pieces.svg?raw'
...
function initGlobe() {
  ...
  const cleaned = globePiecesSvgRaw
...
function dropPiece(id: string, globeRect: DOMRect, velocityMult = 1) {
  ...
  if (verts.length >= 3) {
    ...
    const cx = hull.reduce((s: number, v: { x: number; y: number }) => s + v.x, 0) / hull.length
    const cy = hull.reduce((s: number, v: { x: number; y: number }) => s + v.y, 0) / hull.length
    body = Matter.Body.create({
      label: 'Piece Body',
      position: { x: cx, y: cy },
      vertices: hull.map((v: { x: number; y: number }) => ({ x: v.x - cx, y: v.y - cy })),
      ...bodyOpts,
    })
  } else {
